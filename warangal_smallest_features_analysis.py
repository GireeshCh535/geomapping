...
def analyze_file_for_smallest_feature(filepath):
    ...
    try:
        ...
        # Analyze geometry types and holes
        geometry_type_counts = defaultdict(int)
        features_with_holes = 0
        max_holes_in_feature = 0
        ...
        print(f"Scanning {feature_count:,} features for smallest and analyzing geometry...")
        
        for idx, feature in enumerate(features):
            # Geometry type analysis
            geom = feature.get('geometry', {})
            geom_type = geom.get('type', 'Unknown')
            geometry_type_counts[geom_type] += 1
        ...
        for geom_type, count in sorted(geometry_type_counts.items()):
            percentage = (count / feature_count * 100) if feature_count else 0
            print(f"  - {geom_type}: {count:,} ({percentage:.1f}%)")
        
        print(f"\nHoles Analysis:")
        print(f"  - Features with holes: {features_with_holes:,}")
        if features_with_holes > 0:
            print(f"  - Max holes in a feature: {max_holes_in_feature}")
        ...
        result = {
            ...
            'geometry_types': dict(geometry_type_counts),
            'features_with_holes': features_with_holes,
            'max_holes_in_feature': max_holes_in_feature,
            'smallest_feature': {
...
def save_results_json(all_results, output_path):
    """Save analysis results to JSON file (analytical data, no full geometries)"""
...
def save_feature_samples_json(all_results, output_path):
    """Save only the smallest feature metadata (compact, single line)"""
    samples = {}
    for result in all_results:
        if result and result['smallest_feature']:
            sf = result['smallest_feature']
            # Save only essential info without full geometry coordinates
            samples[result['filename']] = {
                'index': sf['index'],
                'geometry_type': sf['geometry_type'],
                'area_approx_sq_deg': sf['area_approx_sq_deg'],
                'properties': sf['properties'],
                'bbox': sf['bbox'],
                'tile_info': {
                    'center': sf['tile_info']['center'] if sf['tile_info'] else None,
                    'width_km': sf['tile_info']['width_km'] if sf['tile_info'] else None,
                    'height_km': sf['tile_info']['height_km'] if sf['tile_info'] else None,
                    'tiles': sf['tile_info']['tiles'] if sf['tile_info'] else None
                }
            }
    ...
    print(f"✅ Feature samples (compact metadata) saved to: {output_file}")
