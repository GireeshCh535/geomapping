#!/usr/bin/env python3
"""
Django management command to insert Ayodhya master plan data
Creates ONE layer (ayodhya_masterplan) with all masterplan files as features
Following the hierarchy: State (Uttar Pradesh) -> City (Ayodhya) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Ayodhya master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Ayodhya masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/uttar-pradesh/ayodhya/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Ayodhya Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_ayodhya_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Ayodhya Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Uttar Pradesh state and Ayodhya city"""
        self.stdout.write('Setting up Uttar Pradesh state and Ayodhya city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Ayodhya masterplan"""
    ...
    def delete_existing_ayodhya_masterplan_data(self):
        """Delete existing Ayodhya masterplan data"""
        self.stdout.write('Deleting existing Ayodhya masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Ayodhya masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='ayodhya_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Ayodhya master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                defaults={
                    ...
                    'description': 'Ayodhya Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Ayodhya Master Plan',
                    'description': 'Comprehensive Ayodhya master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Ayodhya Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Ayodhya"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Ayodhya"""
