...
Creates ONE layer (chennai_master_plan) with all masterplan files as features
Following the hierarchy: State (Tamil Nadu) -> City (Chennai) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Chennai master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Chennai masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/tamil_nadu/chennai/chennai_master_plan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Chennai Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_chennai_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Chennai Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Tamil Nadu state and Chennai city"""
        self.stdout.write('Setting up Tamil Nadu state and Chennai city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Chennai masterplan"""
    ...
    def delete_existing_chennai_masterplan_data(self):
        """Delete existing Chennai masterplan data"""
        self.stdout.write('Deleting existing Chennai masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Chennai masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='chennai-master-plan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Chennai master plan layer...')
        ...
        layer_group, created = LayerGroup.objects.get_or_create(
            ...
            defaults={
                ...
                'description': 'Chennai Metropolitan Development Authority master plan data',
        ...
        self.master_plan_layer, created = DataLayer.objects.get_or_create(
            ...
            defaults={
                'name': 'Chennai Master Plan',
                'description': 'Comprehensive Chennai master plan including all boundaries and land use zones',
                'category': self.categories['PLANNING'],
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Chennai"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Chennai"""
