"""
Django management command to insert Pithampur master plan data
Creates ONE layer (pithampur_master_plan) with all masterplan files as features
Following the hierarchy: State (Madhya Pradesh) -> City (Pithampur) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Pithampur master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Pithampur masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/madhya-pradesh/pithampur/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Pithampur Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_pithampur_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Pithampur Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Madhya Pradesh state and Pithampur city"""
        self.stdout.write('Setting up Madhya Pradesh state and Pithampur city...')
        
        # Create or get Madhya Pradesh state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Pithampur masterplan"""
    ...
    def delete_existing_pithampur_masterplan_data(self):
        """Delete existing Pithampur masterplan data"""
        self.stdout.write('Deleting existing Pithampur masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Pithampur masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='pithampur_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Pithampur master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='pithampur_masterplan',
                defaults={
                    'name': 'Pithampur Master Plan',
                    'description': 'Pithampur Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Pithampur Master Plan',
                    'description': 'Comprehensive Pithampur master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Pithampur Development Authority',
                }
            )
    ...
    def determine_category_from_filename(self, filename):
        ...
        if any(term in filename_lower for term in ['boundary', 'boundaries', 'planning_area_boundary']):
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Pithampur"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Pithampur"""
