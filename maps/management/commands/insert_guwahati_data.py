#!/usr/bin/env python3
"""
Django management command to insert Guwahati master plan data
Creates ONE layer (guwahati_masterplan) with all masterplan files as features
Following the hierarchy: State (Assam) -> City (Guwahati) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Guwahati master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Guwahati masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/assam/guwahati/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Guwahati Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_guwahati_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Guwahati Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Assam state and Guwahati city"""
        self.stdout.write('Setting up Assam state and Guwahati city...')
        
        # Create or get Assam state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Guwahati masterplan"""
    ...
    def delete_existing_guwahati_masterplan_data(self):
        """Delete existing Guwahati masterplan data"""
        self.stdout.write('Deleting existing Guwahati masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Guwahati masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='guwahati_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Guwahati master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='guwahati_masterplan',
                defaults={
                    'name': 'Guwahati Master Plan',
                    'description': 'Guwahati Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Guwahati Master Plan',
                    'description': 'Comprehensive Guwahati master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Guwahati Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Guwahati"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Guwahati"""
