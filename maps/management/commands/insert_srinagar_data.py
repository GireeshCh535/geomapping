#!/usr/bin/env python3
"""
Django management command to insert Srinagar master plan data
Creates ONE layer (srinagar_masterplan) with all masterplan files as features
Following the hierarchy: State (Jammu & Kashmir) -> City (Srinagar) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Srinagar master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Srinagar masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/jammu-kashmir/srinagar/srinagar_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Srinagar Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_srinagar_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Srinagar Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Jammu & Kashmir state and Srinagar city"""
        self.stdout.write('Setting up Jammu & Kashmir state and Srinagar city...')
        
        # Create or get Jammu & Kashmir state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Srinagar masterplan"""
    ...
    def delete_existing_srinagar_masterplan_data(self):
        """Delete existing Srinagar masterplan data"""
        self.stdout.write('Deleting existing Srinagar masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Srinagar masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='srinagar_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Srinagar master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='srinagar_masterplan',
                defaults={
                    'name': 'Srinagar Master Plan',
                    'description': 'Srinagar Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Srinagar Master Plan',
                    'description': 'Comprehensive Srinagar master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Srinagar Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Srinagar"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Srinagar"""
