#!/usr/bin/env python3
"""
Django management command to insert Tirupati master plan data
Creates ONE layer (tirupati_masterplan) with all masterplan files as features
Following the hierarchy: State (Andhra Pradesh) -> City (Tirupati) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Tirupati master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Tirupati masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/andhra_pradesh/tirupati/tirupati_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Tirupati Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_tirupati_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Tirupati Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Andhra Pradesh state and Tirupati city"""
        self.stdout.write('Setting up Andhra Pradesh state and Tirupati city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Tirupati masterplan"""
    ...
    def delete_existing_tirupati_masterplan_data(self):
        """Delete existing Tirupati masterplan data"""
        self.stdout.write('Deleting existing Tirupati masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Tirupati masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='tirupati_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Tirupati master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='tirupati_masterplan',
                defaults={
                    'name': 'Tirupati Master Plan',
                    'description': 'Tirupati Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Tirupati Master Plan',
                    'description': 'Comprehensive Tirupati master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Tirupati Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Tirupati"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Tirupati"""
