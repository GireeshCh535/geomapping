#!/usr/bin/env python3
"""
Django management command to insert Lucknow master plan data
Creates ONE layer (lucknow_masterplan) with all masterplan files as features
Following the hierarchy: State (Uttar Pradesh) -> City (Lucknow) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Lucknow master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Lucknow masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/uttar-pradesh/lucknow/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Lucknow Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_lucknow_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Lucknow Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Uttar Pradesh state and Lucknow city"""
        self.stdout.write('Setting up Uttar Pradesh state and Lucknow city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Lucknow masterplan"""
    ...
    def delete_existing_lucknow_masterplan_data(self):
        """Delete existing Lucknow masterplan data"""
        self.stdout.write('Deleting existing Lucknow masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Lucknow masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='lucknow_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Lucknow master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='lucknow_masterplan',
                defaults={
                    'name': 'Lucknow Master Plan',
                    'description': 'Lucknow Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Lucknow Master Plan',
                    'description': 'Comprehensive Lucknow master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Lucknow Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Lucknow"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Lucknow"""
