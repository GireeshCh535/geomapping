#!/usr/bin/env python3
"""
Generic Django management command to insert masterplan data for any city
Takes city/state details as command-line arguments instead of hardcoding
This replaces the need for 31 separate insert_*_data.py files

Usage:
    python manage.py insert_masterplan_data \
        --city-name "Vadodara" \
        --city-slug "vadodara" \
        --state-name "Gujarat" \
        --state-code "GJ" \
        --state-slug "gujarat" \
        --data-dir "data/gujarat/vadodara/vadodara_masterplan/" \
        --city-lat 22.3072 \
        --city-lng 73.1812 \
        --state-lat 23.0225 \
        --state-lng 72.5714 \
        --authority "Vadodara Urban Development Authority" \
        --delete-existing
"""
...
class Command(BaseCommand):
    help = 'Insert masterplan data for any city (generic/reusable command)'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            required=True,
            help='City name (e.g., "Vadodara", "Varanasi")',
        )
        parser.add_argument(
            '--city-slug',
            type=str,
            required=True,
            help='City slug (e.g., "vadodara", "varanasi")',
        )
        parser.add_argument(
            '--state-name',
            type=str,
            required=True,
            help='State name (e.g., "Gujarat", "Uttar Pradesh")',
        )
        parser.add_argument(
            '--state-code',
            type=str,
            required=True,
            help='State code (e.g., "GJ", "UP")',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
        ...
        parser.add_argument(
            '--state-slug',
            type=str,
            help='State slug (auto-generated from state-name if not provided)',
        )
        parser.add_argument(
            '--city-lat',
            type=float,
            help='City center latitude',
        )
        parser.add_argument(
            '--city-lng',
            type=float,
            help='City center longitude',
        )
        parser.add_argument(
            '--state-lat',
            type=float,
            help='State center latitude',
        )
        parser.add_argument(
            '--state-lng',
            type=float,
            help='State center longitude',
        )
        parser.add_argument(
            '--authority',
            type=str,
            help='Development authority name (defaults to "{city} Development Authority")',
        )
        parser.add_argument(
            '--min-zoom',
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=7,
            help='State default zoom level (default: 7)',
        )
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing masterplan data before inserting new data',
        )
        parser.add_argument(
            '--exclude',
            type=str,
            default='',
            help='Comma-separated substrings: exclude files whose name contains any of these',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.state_slug = options.get('state_slug') or slugify(self.state_name)
        self.city_lat = options.get('city_lat')
        self.city_lng = options.get('city_lng')
        self.state_lat = options.get('state_lat')
        self.state_lng = options.get('state_lng')
        self.authority = options.get('authority') or f"{self.city_name} Development Authority"
        self.min_zoom = options['min_zoom']
        self.max_zoom = options['max_zoom']
        self.state_default_zoom = options['state_default_zoom']
        ...
        self.stdout.write(
            self.style.SUCCESS(f'🚀 Starting {self.city_name} Masterplan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✅ {self.city_name} Masterplan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup state and city with provided parameters"""
        self.stdout.write(f'Setting up {self.state_name} state and {self.city_name} city...')
        
        # Create or get State
        state_defaults = {
            'name': self.state_name,
            'slug': self.state_slug,
            'default_zoom': self.state_default_zoom,
            'is_active': True
        }
        
        # Add coordinates if provided
        if self.state_lat is not None:
            state_defaults['center_lat'] = self.state_lat
        if self.state_lng is not None:
            state_defaults['center_lng'] = self.state_lng
        
        # Try to get by slug first, then by code
        try:
            self.state = State.objects.get(slug=self.state_slug)
            created = False
            ...
            if created:
                ...
                self.stdout.write(f'  ✅ Found existing state by code: {self.state.name}')
        ...
        if self.city_lat is not None:
            city_defaults['center_lat'] = self.city_lat
        if self.city_lng is not None:
            city_defaults['center_lng'] = self.city_lng
    ...
    def setup_layer_categories(self):
        """Setup layer categories for masterplan"""
    ...
    def delete_existing_masterplan_data(self):
        """Delete existing masterplan data"""
        self.stdout.write(f'Deleting existing {self.city_name} masterplan data...')
        
        # Delete the master plan layer and all its features
        deleted_count = 0
        
        # Build the layer slug
        layer_slug = f'{self.city_slug}_masterplan'
        ...
        try:
            ...
            self.stdout.write(f'  ℹ️ No existing {self.city_name} masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug=layer_slug).delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write(f'\nCreating and populating {self.city_name} master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug=layer_slug,
                defaults={
                    'name': f'{self.city_name} Master Plan',
                    'description': f'{self.authority} master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': f'{self.city_name} Master Plan',
                    'description': f'Comprehensive {self.city_name} master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
