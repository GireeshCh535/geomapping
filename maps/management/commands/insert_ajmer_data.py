"""
Django management command to insert Ajmer master plan data
Creates ONE layer (ajmer_master_plan) with all masterplan files as features
Following the hierarchy: State (Rajasthan) -> City (Ajmer) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Ajmer master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Ajmer masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/rajasthan/ajmer/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Ajmer Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_ajmer_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Ajmer Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Rajasthan state and Ajmer city"""
        self.stdout.write('Setting up Rajasthan state and Ajmer city...')
        
        # Create or get Rajasthan state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Ajmer masterplan"""
    ...
    def delete_existing_ajmer_masterplan_data(self):
        """Delete existing Ajmer masterplan data"""
        self.stdout.write('Deleting existing Ajmer masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Ajmer masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='ajmer_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Ajmer master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='ajmer_masterplan',
                defaults={
                    'name': 'Ajmer Master Plan',
                    'description': 'Ajmer Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Ajmer Master Plan',
                    'description': 'Comprehensive Ajmer master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Ajmer Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Ajmer"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Ajmer"""
