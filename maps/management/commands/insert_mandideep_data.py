#!/usr/bin/env python3
"""
Django management command to insert Mandideep master plan data
Creates ONE layer (mandideep_masterplan) with all masterplan files as features
Following the hierarchy: State (Madhya Pradesh) -> City (Mandideep) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Mandideep master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Mandideep masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/madhya-pradesh/mandideep/mandideep_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Mandideep Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_mandideep_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Mandideep Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Madhya Pradesh state and Mandideep city"""
        self.stdout.write('Setting up Madhya Pradesh state and Mandideep city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Mandideep masterplan"""
    ...
    def delete_existing_mandideep_masterplan_data(self):
        """Delete existing Mandideep masterplan data"""
        self.stdout.write('Deleting existing Mandideep masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Mandideep masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='mandideep_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Mandideep master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='mandideep_masterplan',
                defaults={
                    'name': 'Mandideep Master Plan',
                    'description': 'Mandideep Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Mandideep Master Plan',
                    'description': 'Comprehensive Mandideep master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Mandideep Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Mandideep"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Mandideep"""
