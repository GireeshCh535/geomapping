#!/usr/bin/env python3
"""
Django management command to insert Patna master plan data
Creates ONE layer (patna_masterplan) with all masterplan files as features
Following the hierarchy: State (Bihar) -> City (Patna) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Patna master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Patna masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/bihar/patna/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Patna Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_patna_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Patna Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Bihar state and Patna city"""
        self.stdout.write('Setting up Bihar state and Patna city...')
        
        # Create or get Bihar state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Patna masterplan"""
    ...
    def delete_existing_patna_masterplan_data(self):
        """Delete existing Patna masterplan data"""
        self.stdout.write('Deleting existing Patna masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Patna masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='patna_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Patna master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='patna_masterplan',
                defaults={
                    'name': 'Patna Master Plan',
                    'description': 'Patna Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Patna Master Plan',
                    'description': 'Comprehensive Patna master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Patna Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Patna"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Patna"""
