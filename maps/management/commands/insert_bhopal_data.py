"""
Django management command to insert Bhopal master plan data
Creates ONE layer (bhopal_master_plan) with all masterplan files as features
Following the hierarchy: State (Madhya Pradesh) -> City (Bhopal) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Bhopal master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Bhopal masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/madhya-pradesh/bhopal/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Bhopal Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_bhopal_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Bhopal Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Madhya Pradesh state and Bhopal city"""
        self.stdout.write('Setting up Madhya Pradesh state and Bhopal city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Bhopal masterplan"""
    ...
    def delete_existing_bhopal_masterplan_data(self):
        """Delete existing Bhopal masterplan data"""
        self.stdout.write('Deleting existing Bhopal masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Bhopal masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='bhopal_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Bhopal master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='bhopal_masterplan',
                defaults={
                    'name': 'Bhopal Master Plan',
                    'description': 'Bhopal Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Bhopal Master Plan',
                    'description': 'Comprehensive Bhopal master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Bhopal Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Bhopal"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Bhopal"""
