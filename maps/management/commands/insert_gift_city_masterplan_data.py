#!/usr/bin/env python3
"""
Django management command to insert GIFT City Masterplan data
Creates ONE layer (gift_city_masterplan) with all masterplan files as features
Following the hierarchy: State (Gujarat) -> City (GIFT City) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert GIFT City masterplan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing GIFT City masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/gujarat/gift-city/gift_city_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting GIFT City Masterplan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_gift_city_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ GIFT City Masterplan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Gujarat state and GIFT City"""
        self.stdout.write('Setting up Gujarat state and GIFT City...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for GIFT City masterplan"""
    ...
    def delete_existing_gift_city_masterplan_data(self):
        """Delete existing GIFT City masterplan data"""
        self.stdout.write('Deleting existing GIFT City masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing GIFT City masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='gift_city_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating GIFT City master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='gift_city_masterplan',
                defaults={
                    'name': 'GIFT City Master Plan',
                    'description': 'GIFT City Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'GIFT City Master Plan',
                    'description': 'Comprehensive GIFT City master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'GIFT City Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for GIFT City"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for GIFT City"""
