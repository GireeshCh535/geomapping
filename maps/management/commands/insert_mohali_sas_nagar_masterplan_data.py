#!/usr/bin/env python3
"""
Django management command to insert Mohali SAS Nagar Masterplan data
Creates ONE layer (mohali_sas_nagar_masterplan) with all masterplan files as features
Following the hierarchy: State (Punjab) -> City (Mohali-SAS Nagar) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Mohali SAS Nagar masterplan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Mohali SAS Nagar masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/punjab/mohali-sas-nagar/mohali_sas_nagar_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Mohali SAS Nagar Masterplan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_mohali_sas_nagar_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Mohali SAS Nagar Masterplan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Punjab state and Mohali-SAS Nagar city"""
        self.stdout.write('Setting up Punjab state and Mohali-SAS Nagar city...')
        
        # Create or get Punjab state
        ...
        # Create or get Mohali-SAS Nagar city
        self.city, created = City.objects.get_or_create(
            slug='mohali-sas-nagar',
            defaults={
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Mohali SAS Nagar masterplan"""
    ...
    def delete_existing_mohali_sas_nagar_masterplan_data(self):
        """Delete existing Mohali SAS Nagar masterplan data"""
        self.stdout.write('Deleting existing Mohali SAS Nagar masterplan data...')
        ...
        try:
            layer = DataLayer.objects.get(
                city=self.city,
                slug='mohali_sas_nagar_masterplan'
            )
            ...
            self.stdout.write('  ℹ️ No existing Mohali SAS Nagar masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='mohali_sas_nagar_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Mohali SAS Nagar master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='mohali_sas_nagar_masterplan',
                defaults={
                    'name': 'Mohali SAS Nagar Master Plan',
                    'description': 'Punjab Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Mohali SAS Nagar Master Plan',
                    'description': 'Comprehensive Mohali SAS Nagar master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Punjab Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Mohali SAS Nagar"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Mohali SAS Nagar"""
