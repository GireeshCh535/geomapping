"""
Django management command to insert Coimbatore master plan data
Creates ONE layer (coimbatore_master_plan) with all masterplan files as features
Following the hierarchy: State (Tamil Nadu) -> City (Coimbatore) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Coimbatore master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Coimbatore masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Coimbatore Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_coimbatore_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Coimbatore Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Tamil Nadu state and Coimbatore city"""
        self.stdout.write('Setting up Tamil Nadu state and Coimbatore city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Coimbatore masterplan"""
    ...
    def delete_existing_coimbatore_masterplan_data(self):
        """Delete existing Coimbatore masterplan data"""
        self.stdout.write('Deleting existing Coimbatore masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Coimbatore masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='coimbatore-master-plan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Coimbatore master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='coimbatore-master-plan',
                defaults={
                    'name': 'Coimbatore Master Plan',
                    'description': 'Coimbatore Municipal Corporation master plan data',
            ...
            city=self.city,
                ...
                defaults={
                    'name': 'Coimbatore Master Plan',
                    'description': 'Comprehensive Coimbatore master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Coimbatore Municipal Corporation',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Coimbatore"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Coimbatore"""
