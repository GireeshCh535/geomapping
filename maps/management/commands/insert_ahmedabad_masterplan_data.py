#!/usr/bin/env python3
"""
Django management command to insert Ahmedabad Masterplan data
Creates ONE layer (ahmedabad_masterplan) with all masterplan files as features
Following the hierarchy: State (Gujarat) -> City (Ahmedabad-Gandhinagar) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Ahmedabad masterplan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Ahmedabad masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/gujarat/ahmedabad-gandhinagar/ahmedabad_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Ahmedabad Masterplan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_ahmedabad_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Ahmedabad Masterplan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Gujarat state and Ahmedabad-Gandhinagar city"""
        self.stdout.write('Setting up Gujarat state and Ahmedabad-Gandhinagar city...')
        
        # Create or get Gujarat state
        ...
        # Create or get Ahmedabad-Gandhinagar city
        self.city, created = City.objects.get_or_create(
            slug='ahmedabad-gandhinagar',
            defaults={
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Ahmedabad masterplan"""
    ...
    def delete_existing_ahmedabad_masterplan_data(self):
        """Delete existing Ahmedabad masterplan data"""
        self.stdout.write('Deleting existing Ahmedabad masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Ahmedabad masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='ahmedabad_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Ahmedabad master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='ahmedabad_masterplan',
                defaults={
                    'name': 'Ahmedabad Master Plan',
                    'description': 'Ahmedabad Urban Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Ahmedabad Master Plan',
                    'description': 'Comprehensive Ahmedabad master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Ahmedabad Urban Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Ahmedabad"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Ahmedabad"""
