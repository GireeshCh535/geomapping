"""
Django management command to insert Cuttack master plan data
Creates ONE layer (cuttack_master_plan) with all masterplan files as features
Following the hierarchy: State (Odisha) -> City (Cuttack) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Cuttack master plan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Cuttack masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/odisha/cuttack/cuttack_master_plan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Cuttack Master Plan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_cuttack_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Cuttack Master Plan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Odisha state and Cuttack city"""
        self.stdout.write('Setting up Odisha state and Cuttack city...')
        
        # Create or get Odisha state
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Cuttack masterplan"""
    ...
    def delete_existing_cuttack_masterplan_data(self):
        """Delete existing Cuttack masterplan data"""
        self.stdout.write('Deleting existing Cuttack masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Cuttack masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='cuttack-masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Cuttack master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='cuttack-masterplan',
                defaults={
                    'name': 'Cuttack Master Plan',
                    'description': 'Cuttack Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Cuttack Master Plan',
                    'description': 'Comprehensive Cuttack master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Cuttack Development Authority, Odisha',
                }
            )
    ...
    def determine_category_from_filename(self, filename):
        ...
        if any(term in filename_lower for term in ['boundary', 'boundaries', 'cma_boundary', 'city_boundary', 'planning_area_boundary']):
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Cuttack"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Cuttack"""
