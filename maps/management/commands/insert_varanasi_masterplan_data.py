#!/usr/bin/env python3
"""
Django management command to insert Varanasi Masterplan data
Creates ONE layer (varanasi_masterplan) with all masterplan files as features
Following the hierarchy: State (Uttar Pradesh) -> City (Varanasi) -> DataLayer -> GeoFeatures from all files
"""
...
class Command(BaseCommand):
    help = 'Insert Varanasi masterplan data into the database'

    def add_arguments(self, parser):
        parser.add_argument(
            '--delete-existing',
            action='store_true',
            help='Delete existing Varanasi masterplan data before inserting new data',
        )
        parser.add_argument(
            '--data-dir',
            type=str,
            default='data/uttar-pradesh/varanasi/varanasi_masterplan/',
            help='Directory containing the master plan data files',
        )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🚀 Starting Varanasi Masterplan Data Insertion')
        )
        ...
        try:
            with transaction.atomic():
                ...
                if options['delete_existing']:
                    self.delete_existing_varanasi_masterplan_data()
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ Varanasi Masterplan Data Insertion Completed Successfully!')
                )
    ...
    def setup_state_and_city(self):
        """Setup Uttar Pradesh state and Varanasi city"""
        self.stdout.write('Setting up Uttar Pradesh state and Varanasi city...')
    ...
    def setup_layer_categories(self):
        """Setup layer categories for Varanasi masterplan"""
    ...
    def delete_existing_varanasi_masterplan_data(self):
        """Delete existing Varanasi masterplan data"""
        self.stdout.write('Deleting existing Varanasi masterplan data...')
        ...
        try:
            ...
            self.stdout.write('  ℹ️ No existing Varanasi masterplan layer found')
        
        # Also delete any layer groups
        LayerGroup.objects.filter(city=self.city, slug='varanasi_masterplan').delete()

    def create_and_populate_master_plan_layer(self):
        """Create ONE master plan layer and populate it with features from all files"""
        self.stdout.write('\nCreating and populating Varanasi master plan layer...')
        ...
        if not all_files:
            ...
            layer_group, created = LayerGroup.objects.get_or_create(
                ...
                slug='varanasi_masterplan',
                defaults={
                    'name': 'Varanasi Master Plan',
                    'description': 'Varanasi Development Authority master plan data',
            ...
            self.master_plan_layer, created = DataLayer.objects.get_or_create(
                ...
                defaults={
                    'name': 'Varanasi Master Plan',
                    'description': 'Comprehensive Varanasi master plan including all boundaries and land use zones',
                    'category': self.categories['PLANNING'],
                    ...
                    'feature_count': 0,
                    'is_true': True,
                    'data_source': 'Varanasi Development Authority',
                }
            )
    ...
    def create_city_layer_styles(self):
        """Create city-specific layer styles for Varanasi"""
    ...
    def create_zone_mappings(self):
        """Create zone mappings for Varanasi"""
