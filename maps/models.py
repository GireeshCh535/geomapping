...
class SyncedLand(models.Model):
    ...
    lgd_slug = models.CharField(max_length=500, blank=True, help_text='LGD slug from API (e.g. 2.7-acre-land-in-agiryal-...)')
    marker_title = models.CharField(max_length=500, blank=True, help_text='Display title from API (e.g. 2 Acres 28 Guntas - ₹100 Lakhs/Acre)')
    marker_id = models.CharField(max_length=80, blank=True, help_text='Icon key from API (e.g. land-10, land-owner-7) for 1acre-icons/{type}/{base|owner|1acre}/{marker_id}.svg')
    payload = models.JSONField(default=dict, help_text='Full list or detail item from API; webhook listing_data stored here as-is')
    synced_at = models.DateTimeField(auto_now=True)
...
class SyncedPlot(models.Model):
    ...
    lgd_slug = models.CharField(max_length=500, blank=True, help_text='LGD slug from API')
    marker_title = models.CharField(max_length=500, blank=True, help_text='Display title from API (e.g. 145 Sq Yards - ₹28800 /Sq Yard)')
    marker_id = models.CharField(max_length=80, blank=True, help_text='Icon key from API (e.g. plot-11, plot-owner-12) for 1acre-icons/{type}/{base|owner|1acre}/{marker_id}.svg')
    payload = models.JSONField(default=dict, help_text='Full list or detail item from API; webhook listing_data stored here as-is')
    synced_at = models.DateTimeField(auto_now=True)
    location_point = models.PointField(geography=True, null=True, blank=True)
    enriched_layers = models.JSONField(
...
class SyncedDeveloperLand(models.Model):
    ...
    description = models.TextField(blank=True)
    lgd_slug = models.CharField(max_length=500, blank=True, help_text='LGD slug from API (e.g. 24.8-acre-land-in-mandakalli-...)')
    payload = models.JSONField(default=dict, help_text='Full list or detail item from API; webhook listing_data stored here as-is')
    synced_at = models.DateTimeField(auto_now=True)
    location_point = models.PointField(geography=True, null=True, blank=True)
    enriched_layers = models.JSONField(
...
class SyncedDeveloperPlot(models.Model):
    ...
    marker_title = models.CharField(max_length=500, blank=True)
    description = models.TextField(blank=True)
    lgd_slug = models.CharField(max_length=500, blank=True, help_text='LGD slug from API (e.g. 5929-sqyd-plot-in-adibatla-...)')
    payload = models.JSONField(default=dict, help_text='Full list or detail item from API; webhook listing_data stored here as-is')
    synced_at = models.DateTimeField(auto_now=True)
    location_point = models.PointField(geography=True, null=True, blank=True)
    enriched_layers = models.JSONField(
