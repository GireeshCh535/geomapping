...
urlpatterns = [
    ...
    # Hierarchical PNG/MVT: Django proxies. <path:layer_slug> supports developer_data/.../id/file.tif style prefixes.
    path('tiles/<slug:state_slug>/<slug:city_slug>/<path:layer_slug>/<int:z>/<int:x>/<int:y>.png',
         views.CloudFrontTileView.as_view(),
         name='cloudfront_tile_png'),
    path('tiles/<slug:state_slug>/<slug:city_slug>/<path:layer_slug>/<int:z>/<int:x>/<int:y>.mvt',
         views.CloudFrontTileView.as_view(),
         name='cloudfront_tile_mvt'),

    path('s3-tiles/<slug:state_slug>/<slug:city_slug>/<path:layer_slug>/<int:z>/<int:x>/<int:y>.png',
         views.S3DirectTileView.as_view(),
         name='s3_direct_tile_png'),
    path('s3-tiles/<slug:state_slug>/<slug:city_slug>/<path:layer_slug>/<int:z>/<int:x>/<int:y>.mvt',
         views.S3DirectTileView.as_view(),
    ...
    # Land/plot MVT: Django proxies (backend fetches S3 or CloudFront).
    path('tiles/land-plot/<int:z>/<int:x>/<int:y>.mvt',
         views.LandPlotLocalTileView.as_view(),
         name='land_plot_local_tile'),

    # All lands/plots as one GeoJSON (load once, show at all zoom levels)
    path('geojson/land-plot/all.geojson',
         views.LandPlotGeoJSONView.as_view(),
         name='land_plot_geojson_all'),

    # HTML test page for land/plot map
