...
def write_rgb_geotiff(rgba_data, transform, bounds, output_file):
    """Write RGB GeoTIFF with LZW compression for smaller file size"""
    print(f"\n[5/6] Writing RGB GeoTIFF with LZW compression...")
    print(f"  Using LZW compression to reduce file size...")
    ...
    # Use LZW compression with tiled writing for better compression
    with rasterio.open(
        output_file,
        'w',
        driver='GTiff',
        ...
        crs='EPSG:4326',
        transform=transform,
        compress='lzw',  # LZW compression - safe for geospatial data
        tiled=True,
        blockxsize=512,
        blockysize=512,
        BIGTIFF='YES'
    ...
    uncompressed_size = (height * width * 4) / (1024 * 1024)
    compression_ratio = (file_size / uncompressed_size) * 100
    
    print(f"\n  ✓ File written: {output_file}")
    print(f"  ✓ File size: {file_size:.2f} MB")
    print(f"  Uncompressed would be: {uncompressed_size:.2f} MB")
    print(f"  Compression ratio: {compression_ratio:.1f}%")
    print(f"  Space saved: {uncompressed_size - file_size:.2f} MB")
...
def main():
    ...
    try:
        ...
        print(f"Compression: LZW (preserves all data while reducing file size)")
