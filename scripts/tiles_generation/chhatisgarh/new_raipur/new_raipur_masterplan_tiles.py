#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from New Raipur Masterplan RGBA GeoTIFF
...
class NewRaipurRGBATileGenerator:
    """
    Generate high-quality PNG tiles from New Raipur Masterplan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/chhatisgarh/new-raipur/new_raipur_masterplan",
                 output_dir: str = "new_raipur_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("New Raipur RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for New Raipur Masterplan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for New Raipur Masterplan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Chhattisgarh - New Raipur Masterplan",
            "sources": {
                "new-raipur-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/chhattisgarh/new_raipur_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "new-raipur-masterplan-layer",
                    "type": "raster",
                    "source": "new-raipur-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Chhattisgarh - New Raipur Masterplan",
            "description": "Master plan tiles for New Raipur, Chhattisgarh",
            "version": "1.0.0",
            "attribution": "New Raipur Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Chhattisgarh - New Raipur Masterplan</title>
...
def main():
    """Main function"""
    logger.info("Starting New Raipur RGBA Masterplan tile generation")
    
    # Initialize generator
    generator = NewRaipurRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("New Raipur RGBA Masterplan tile generation completed!")
