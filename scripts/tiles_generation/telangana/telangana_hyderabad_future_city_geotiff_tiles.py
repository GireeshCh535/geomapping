#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Future City Hyderabad GeoTIFF
...
class FutureCityHyderabadRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Future City Hyderabad RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/Telangana/Hyderabad/future-city",
                 output_dir: str = "hyderabad_future_city_geotiff_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Future City Hyderabad RGBA Tile Generator initialized")
    ...
    def reproject_geotiff_to_wgs84(self, geotiff_path):
        ...
        with rasterio.open(geotiff_path) as src:
            ...
            # Since the GeoTIFF is already in WGS84, we can read it directly
            if src.crs == 'EPSG:4326':
                logger.info("GeoTIFF is already in WGS84, reading directly")
                ...
                wgs84_transform = src.transform
                
                return data[0], data[1], data[2], data[3], wgs84_bounds, wgs84_transform
            else:
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Future City Hyderabad GeoTIFF"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Future City Hyderabad GeoTIFF")
    ...
    def render_data_to_tile(self, wgs84_data_r, wgs84_data_g, wgs84_data_b, wgs84_data_a, wgs84_bounds, wgs84_transform, tile_bounds, draw):
        ...
        try:
            ...
            # Get data dimensions
            if len(wgs84_data_r.shape) == 2:
                height, width = wgs84_data_r.shape
            else:
                height, width = wgs84_data_r.shape[1], wgs84_data_r.shape[2]
                ...
                for tile_x in range(0, 256, 1):
                    ...
                    if 0 <= data_x < width and 0 <= data_y < height:
                        if len(wgs84_data_r.shape) == 2:
                            ...
                            r = int(wgs84_data_r[0, data_y, data_x])
                            g = int(wgs84_data_g[0, data_y, data_x])
                            b = int(wgs84_data_b[0, data_y, data_x])
                            a = int(wgs84_data_a[0, data_y, data_x])
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Telangana - Future City Hyderabad (GeoTIFF)",
            "sources": {
                "future-city-hyderabad-geotiff": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/telangana/hyderabad/future_city_geotiff/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "future-city-hyderabad-geotiff-layer",
                    "type": "raster",
                    "source": "future-city-hyderabad-geotiff",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Telangana - Future City Hyderabad (GeoTIFF)",
            "description": "Future City Development Authority (FCDA) GeoTIFF tiles for Hyderabad",
            "version": "1.0.0",
            "attribution": "FCDA",
            "template": "",
            "legend": "",
...
<html>
<head>
    <title>Telangana - Future City Hyderabad (GeoTIFF)</title>
...
def main():
    """Main function"""
    logger.info("Starting Future City Hyderabad GeoTIFF tile generation")
    
    # Initialize generator
    generator = FutureCityHyderabadRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=8, max_zoom=16)
    
    logger.info("Future City Hyderabad GeoTIFF tile generation completed!")
