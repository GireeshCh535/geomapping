#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Warangal Masterplan RGBA GeoTIFF
...
class ThreadedWarangalMasterplanTileGenerator:
    """
    Generate high-quality PNG tiles from Warangal Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """

    def __init__(self, data_path: str = "warangal_masterplan_zoom15.tif",
                 output_dir: str = "warangal_masterplan_tiles_threaded",
        ...
        logger.info(f"Threaded Warangal Masterplan Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Warangal Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Warangal Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Telangana - Warangal Masterplan (Threaded)",
            "sources": {
                "warangal-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/telangana/warangal_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "warangal-masterplan-layer",
                    "type": "raster",
                    "source": "warangal-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Telangana - Warangal Masterplan (Threaded)",
            "description": "Master plan tiles for Telangana - Warangal (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Telangana Planning Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Telangana - Warangal Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Warangal Masterplan tile generation")

    # Initialize generator with configurable workers
    generator = ThreadedWarangalMasterplanTileGenerator(max_workers=8)

    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)

    logger.info("Threaded Warangal Masterplan tile generation completed!")
