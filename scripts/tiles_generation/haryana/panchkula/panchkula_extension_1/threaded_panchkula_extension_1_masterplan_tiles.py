#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Panchkula Extension 1 Masterplan RGBA GeoTIFF
...
class ThreadedPanchkulaExtension1RGBATileGenerator:
    """
    Generate high-quality PNG tiles from Panchkula Extension 1 Masterplan RGBA GeoTIFF with multi-threading
    ...
    def __init__(self, data_dir: str = "data/haryana/panchkula/panchkula_extension_1_masterplan",
        ...
        logger.info(f"Threaded Panchkula Extension 1 RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Panchkula Extension 1 Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Panchkula Extension 1 Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Haryana - Panchkula Extension 1 Masterplan (Threaded)",
            "sources": {
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Haryana - Panchkula Extension 1 Masterplan (Threaded)",
            "description": "Master plan tiles for Haryana - Panchkula Extension 1 (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
def main():
    """Main function"""
    logger.info("Starting Threaded Panchkula Extension 1 RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedPanchkulaExtension1RGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Panchkula Extension 1 RGBA Masterplan tile generation completed!")
