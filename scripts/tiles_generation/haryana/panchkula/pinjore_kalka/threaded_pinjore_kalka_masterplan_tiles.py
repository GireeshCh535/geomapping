#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Pinjore Kalka Masterplan RGBA GeoTIFF
...
class ThreadedPinjoreKalkaRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Pinjore Kalka Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/haryana/panchkula/pinjore_kalka_masterplan",
                 output_dir: str = "pinjore_kalka_masterplan_tiles",
        ...
        logger.info(f"Threaded Pinjore Kalka RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Pinjore Kalka Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Pinjore Kalka Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Haryana - Pinjore Kalka Masterplan (Threaded)",
            "sources": {
                "pinjore-kalka-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/haryana/pinjore_kalka_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "pinjore-kalka-masterplan-layer",
                    "type": "raster",
                    "source": "pinjore-kalka-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Haryana - Pinjore Kalka Masterplan (Threaded)",
            "description": "Master plan tiles for Haryana - Pinjore Kalka (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Panchkula Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Haryana - Pinjore Kalka Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Pinjore Kalka RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedPinjoreKalkaRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Pinjore Kalka RGBA Masterplan tile generation completed!")
