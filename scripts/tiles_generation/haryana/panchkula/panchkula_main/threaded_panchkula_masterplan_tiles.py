#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Panchkula Masterplan RGBA GeoTIFF
...
class ThreadedPanchkulaRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Panchkula Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/haryana/panchkula/panchukula_masterplan",
                 output_dir: str = "panchkula_masterplan_tiles",
        ...
        logger.info(f"Threaded Panchkula RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Panchkula Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Panchkula Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Haryana - Panchkula Masterplan (Threaded)",
            "sources": {
                "panchkula-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/haryana/panchkula_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "panchkula-masterplan-layer",
                    "type": "raster",
                    "source": "panchkula-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Haryana - Panchkula Masterplan (Threaded)",
            "description": "Master plan tiles for Haryana - Panchkula (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Haryana - Panchkula Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Panchkula RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedPanchkulaRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Panchkula RGBA Masterplan tile generation completed!")
