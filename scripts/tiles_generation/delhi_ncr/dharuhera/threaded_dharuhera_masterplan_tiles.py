#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Dharuhera Masterplan RGBA GeoTIFF
...
class ThreadedDharuheraRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Dharuhera Masterplan RGBA GeoTIFF with multi-threading
    ...
    def __init__(self, data_dir: str = "data/delhi_ncr/dharuhera/dharuhera_masterplan",
        ...
        logger.info(f"Threaded Dharuhera RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Dharuhera Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Dharuhera Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Dharuhera Masterplan (Threaded)",
            "sources": {
                "dharuhera-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/dharuhera_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "dharuhera-masterplan-layer",
                    "type": "raster",
                    "source": "dharuhera-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Dharuhera Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Dharuhera (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Delhi-NCR - Dharuhera Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Dharuhera RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedDharuheraRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Dharuhera RGBA Masterplan tile generation completed!")
