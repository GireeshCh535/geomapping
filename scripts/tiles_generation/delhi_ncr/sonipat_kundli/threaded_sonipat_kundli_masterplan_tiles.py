#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Sonipat Kundli Masterplan RGBA GeoTIFF
...
class ThreadedSonipatKundliRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Sonipat Kundli Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/sonipat_kundli/sonipat_kundli_masterplan",
                 output_dir: str = "sonipat_kundli_masterplan_tiles",
        ...
        logger.info(f"Threaded Sonipat Kundli RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Sonipat Kundli Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Sonipat Kundli Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Sonipat Kundli Masterplan (Threaded)",
            "sources": {
                "sonipat-kundli-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/sonipat_kundli_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "sonipat-kundli-masterplan-layer",
                    "type": "raster",
                    "source": "sonipat-kundli-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Sonipat Kundli Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Sonipat Kundli (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Delhi-NCR - Sonipat Kundli Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Sonipat Kundli RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedSonipatKundliRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Sonipat Kundli RGBA Masterplan tile generation completed!")
