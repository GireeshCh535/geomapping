#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Ghaziabad Masterplan RGBA GeoTIFF
...
class ThreadedGhaziabadRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Ghaziabad Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/ghaziabad/ghaziabad_masterplan",
                 output_dir: str = "ghaziabad_masterplan_tiles",
        ...
        logger.info(f"Threaded Ghaziabad RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Ghaziabad Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Ghaziabad Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Ghaziabad Masterplan (Threaded)",
            "sources": {
                "ghaziabad-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/ghaziabad_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "ghaziabad-masterplan-layer",
                    "type": "raster",
                    "source": "ghaziabad-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Ghaziabad Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Ghaziabad (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Ghaziabad Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Delhi-NCR - Ghaziabad Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Ghaziabad RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedGhaziabadRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Ghaziabad RGBA Masterplan tile generation completed!")
