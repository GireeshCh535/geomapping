#!/usr/bin/env python3
"""
Delhi Master Plan - SEAMLESS COMPLETE TILES
...
class DelhiSeamlessTiles:
    ...
    def load_geojson_files(self):
        """Load all Delhi GeoJSON files"""
        print("\n" + "="*80)
        print("LOADING DELHI GEOJSON DATA")
        print("="*80)
...
<head>
  <meta charset="utf-8" />
  <title>Delhi Master Plan - Seamless Tiles</title>
...
<body>
  ...
  <script>
    ...
    L.tileLayer('./{{z}}/{{x}}/{{y}}.png', {{
      ...
      maxZoom: 18,
      opacity: 0.9,
      attribution: 'Delhi Master Plan'
    }}).addTo(map);
    
    const info = L.control({{position: 'topright'}});
    info.onAdd = function() {{
      this._div = L.DomUtil.create('div', 'info');
      this._div.innerHTML = '<b>Delhi Master Plan</b><br/>Seamless tiles - no boundaries<br/>Zoom: ' + map.getZoom();
      return this._div;
    }};
    info.addTo(map);
    
    map.on('zoomend', function() {{
      info._div.innerHTML = '<b>Delhi Master Plan</b><br/>Seamless tiles - no boundaries<br/>Zoom: ' + map.getZoom();
    }});
  </script>
</body>
</html>"""
...
def main():
    ...
    possible_paths = [
        Path('data/delhi_ncr/master_plan'),
        Path('/Users/rohitboni/Downloads/All_files/project/1acre/geomapping_full/geomapping/data/delhi_ncr/master_plan'),
    ]
    ...
    if data_dir is None:
        print("Enter path to Delhi master_plan directory:")
    ...
    generator = DelhiSeamlessTiles(data_dir, output_dir)
    ...
    print(f"\n💡 To view: cd {output_dir} && python3 -m http.server 8002")
