#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Arogya Dham Badsa Masterplan RGBA GeoTIFF
...
class ThreadedArogyaDhamBadsaRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Arogya Dham Badsa Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/arogya_dham_badsa/arogya_dham_badsa_masterplan",
                 output_dir: str = "arogya_dham_badsa_masterplan_tiles",
        ...
        logger.info(f"Threaded Arogya Dham Badsa RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Arogya Dham Badsa Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Arogya Dham Badsa Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Arogya Dham Badsa Masterplan (Threaded)",
            "sources": {
                "arogya-dham-badsa-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/arogya_dham_badsa_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "arogya-dham-badsa-masterplan-layer",
                    "type": "raster",
                    "source": "arogya-dham-badsa-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Arogya Dham Badsa Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Arogya Dham Badsa (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Haryana Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Delhi-NCR - Arogya Dham Badsa Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Arogya Dham Badsa RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedArogyaDhamBadsaRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Arogya Dham Badsa RGBA Masterplan tile generation completed!")
