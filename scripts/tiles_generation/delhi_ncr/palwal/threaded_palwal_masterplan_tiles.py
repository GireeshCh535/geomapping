#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Palwal Masterplan RGBA GeoTIFF
...
class ThreadedPalwalRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Palwal Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/palwal/palwal_masterplan",
                 output_dir: str = "palwal_masterplan_tiles",
        ...
        logger.info(f"Threaded Palwal RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Palwal Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Palwal Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Palwal Masterplan (Threaded)",
            "sources": {
                "palwal-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/palwal_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "palwal-masterplan-layer",
                    "type": "raster",
                    "source": "palwal-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Palwal Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Palwal (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Delhi-NCR - Palwal Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Palwal RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedPalwalRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Palwal RGBA Masterplan tile generation completed!")
