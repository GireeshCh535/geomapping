#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Loni Masterplan RGBA GeoTIFF
...
class ThreadedLoniRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Loni Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/loni/loni_masterplan",
                 output_dir: str = "loni_masterplan_tiles",
        ...
        logger.info(f"Threaded Loni RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Loni Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Loni Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Loni Masterplan (Threaded)",
            "sources": {
                "loni-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/loni_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "loni-masterplan-layer",
                    "type": "raster",
                    "source": "loni-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Loni Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Loni (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Delhi-NCR - Loni Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Loni RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedLoniRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Loni RGBA Masterplan tile generation completed!")
