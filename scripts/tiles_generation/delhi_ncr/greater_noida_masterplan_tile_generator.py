#!/usr/bin/env python3
"""
Greater Noida Master Plan - SEAMLESS COMPLETE TILES
...
class GreaterNoidaSeamlessTiles:
    ...
    def get_color_map(self):
        ...
        return {
            ...
            "SECTOR LAYOUT": {'fill': '#FFD37F', 'outline': '#CCAA66'},
            "SECTOR LAYOUT ALLOTED": {'fill': None, 'outline': '#000000'},
            "SECTOR LAYOUT RESERVED": {'fill': '#FF7F7F', 'outline': '#CC6666'},
            "SECTOR LAYOUT VACANT": {'fill': '#FFD37F', 'outline': '#CCAA66'},
            ...
            "SECTOR 19_22": {'fill': '#FFFF00', 'outline': '#CCCC00'},
    ...
    def load_geojson_files(self):
        """Load all Greater Noida GeoJSON files"""
        print("\n" + "="*80)
        print("LOADING GREATER NOIDA GEOJSON DATA")
        print("="*80)
    ...
    def render_tile_seamless(self, tile):
        """
        SEAMLESS RENDERING for Greater Noida:
    ...
    def generate_tiles(self, min_zoom=7, max_zoom=18):
        """Generate seamless tiles for Greater Noida"""
        print(f"\n{'='*80}")
        print(f"GENERATING GREATER NOIDA TILES (Zoom {min_zoom}-{max_zoom})")
    ...
    def generate_html_viewer(self):
        """Generate viewer for Greater Noida"""
        bounds = self.get_bounds()
...
<head>
  <meta charset="utf-8" />
  <title>Greater Noida Master Plan - Seamless Tiles</title>
...
<body>
  ...
  <script>
    ...
    L.tileLayer('./{{z}}/{{x}}/{{y}}.png', {{
      ...
      maxZoom: 18,
      opacity: 0.9,
      attribution: 'Greater Noida Master Plan'
    }}).addTo(map);
    
    const info = L.control({{position: 'topright'}});
    info.onAdd = function() {{
      this._div = L.DomUtil.create('div', 'info');
      this._div.innerHTML = '<b>Greater Noida Master Plan</b><br/>Seamless tiles - no boundaries<br/>Zoom: ' + map.getZoom();
      return this._div;
    }};
    info.addTo(map);
    
    map.on('zoomend', function() {{
      info._div.innerHTML = '<b>Greater Noida Master Plan</b><br/>Seamless tiles - no boundaries<br/>Zoom: ' + map.getZoom();
    }});
  </script>
</body>
</html>"""
...
def main():
    ...
    possible_paths = [
        Path('data/delhi_ncr/greater_noida/master_plan'),
        Path('/Users/rohitboni/Downloads/All_files/project/1acre/geomapping_full/geomapping/data/delhi_ncr/greater_noida/master_plan'),
    ]
    ...
    if data_dir is None:
        print("Enter path to Greater Noida master_plan directory:")
    ...
    output_dir = Path('./greater_noida_tiles_seamless')
    
    print("="*80)
    print("GREATER NOIDA MASTER PLAN - SEAMLESS TILE GENERATOR")
    print("✅ Properly handles polygon holes/interior rings")
    print("="*80)
    print(f"Input:  {data_dir}")
    print(f"Output: {output_dir}")
    
    generator = GreaterNoidaSeamlessTiles(data_dir, output_dir)
    ...
    print(f"\n💡 To view: cd {output_dir} && python3 -m http.server 8004")
