#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Prithal Masterplan RGBA GeoTIFF
...
class ThreadedPrithalRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Prithal Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/delhi_ncr/prithal/prithal_masterplan",
                 output_dir: str = "prithal_masterplan_tiles",
        ...
        logger.info(f"Threaded Prithal RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Prithal Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Prithal Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Delhi-NCR - Prithal Masterplan (Threaded)",
            "sources": {
                "prithal-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/delhi-ncr/prithal_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "prithal-masterplan-layer",
                    "type": "raster",
                    "source": "prithal-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Delhi-NCR - Prithal Masterplan (Threaded)",
            "description": "Master plan tiles for Delhi-NCR - Prithal (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Delhi-NCR - Prithal Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Prithal RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedPrithalRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Prithal RGBA Masterplan tile generation completed!")
