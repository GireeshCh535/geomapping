#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Lucknow Masterplan RGBA GeoTIFF
...
class LucknowRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Lucknow Masterplan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/uttar-pradesh/lucknow",
                 output_dir: str = "lucknow_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Lucknow RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Lucknow Masterplan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Lucknow Masterplan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Uttar Pradesh - Lucknow Masterplan",
            "sources": {
                "lucknow-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/uttar-pradesh/lucknow_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "lucknow-masterplan-layer",
                    "type": "raster",
                    "source": "lucknow-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Uttar Pradesh - Lucknow Masterplan",
            "description": "Master plan tiles for Lucknow, Uttar Pradesh",
            "version": "1.0.0",
            "attribution": "Lucknow Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Uttar Pradesh - Lucknow Masterplan</title>
...
def main():
    """Main function"""
    logger.info("Starting Lucknow RGBA Masterplan tile generation")
    
    # Initialize generator
    generator = LucknowRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=17, max_zoom=18)
    
    logger.info("Lucknow RGBA Masterplan tile generation completed!")
