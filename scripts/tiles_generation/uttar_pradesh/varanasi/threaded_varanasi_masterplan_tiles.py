#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Varanasi Masterplan RGBA GeoTIFF
...
class ThreadedVaranasiRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Varanasi Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/uttar-pradesh/varanasi/varanasi_masterplan",
                 output_dir: str = "varanasi_masterplan_tiles",
        ...
        logger.info(f"Threaded Varanasi RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Varanasi Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Varanasi Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Uttar Pradesh - Varanasi Masterplan (Threaded)",
            "sources": {
                "varanasi-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/uttar-pradesh/varanasi_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "varanasi-masterplan-layer",
                    "type": "raster",
                    "source": "varanasi-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Uttar Pradesh - Varanasi Masterplan (Threaded)",
            "description": "Master plan tiles for Uttar Pradesh - Varanasi (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Varanasi Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Uttar Pradesh - Varanasi Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Varanasi RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedVaranasiRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Varanasi RGBA Masterplan tile generation completed!")
