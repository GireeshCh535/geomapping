#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Visakhapatnam Masterplan RGBA GeoTIFF
...
class ThreadedVisakhapatnamMasterplanTileGenerator:
    """
    Generate high-quality PNG tiles from Visakhapatnam Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """

    def __init__(self, data_path: str = "visakhapatnam_masterplan_zoom16_compressed.tif",
                 output_dir: str = "visakhapatnam_masterplan_tiles_threaded",
        ...
        logger.info(f"Threaded Visakhapatnam Masterplan Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Visakhapatnam Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Visakhapatnam Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Andhra Pradesh - Visakhapatnam Masterplan (Threaded)",
            "sources": {
                "visakhapatnam-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/andhra-pradesh/visakhapatnam_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "visakhapatnam-masterplan-layer",
                    "type": "raster",
                    "source": "visakhapatnam-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Andhra Pradesh - Visakhapatnam Masterplan (Threaded)",
            "description": "Master plan tiles for Andhra Pradesh - Visakhapatnam (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Andhra Pradesh - Visakhapatnam Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Visakhapatnam Masterplan tile generation")

    # Initialize generator with configurable workers
    generator = ThreadedVisakhapatnamMasterplanTileGenerator(max_workers=8)

    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)

    logger.info("Threaded Visakhapatnam Masterplan tile generation completed!")
