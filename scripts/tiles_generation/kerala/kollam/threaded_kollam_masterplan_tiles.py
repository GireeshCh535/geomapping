#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Kollam Masterplan RGBA GeoTIFF
...
class ThreadedKollamRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Kollam Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/kerela/kollam/kollam_masterplan",
                output_dir: str = "kollam_masterplan_tiles",
        ...
        logger.info(f"Threaded Kollam RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Kollam Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Kollam Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Kerala - Kollam Masterplan (Threaded)",
            "sources": {
                "kollam-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/kerala/kollam_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "kollam-masterplan-layer",
                    "type": "raster",
                    "source": "kollam-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Kerala - Kollam Masterplan (Threaded)",
            "description": "Master plan tiles for Kerala - Kollam (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Kollam Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Kerala - Kollam Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Kollam RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedKollamRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Kollam RGBA Masterplan tile generation completed!")
