#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Kozhikode Masterplan RGBA GeoTIFF
...
class ThreadedKozhikodeRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Kozhikode Masterplan RGBA GeoTIFF with multi-threading
    ...
    def __init__(self, data_dir: str = "data/kerela/kozhikode/kozhikode_masterplan",
        ...
        logger.info(f"Threaded Kozhikode RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Kozhikode Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Kozhikode Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Kerala - Kozhikode Masterplan (Threaded)",
            "sources": {
                "kozhikode-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/kerala/kozhikode_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "kozhikode-masterplan-layer",
                    "type": "raster",
                    "source": "kozhikode-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Kerala - Kozhikode Masterplan (Threaded)",
            "description": "Master plan tiles for Kerala - Kozhikode (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Kozhikode Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Kerala - Kozhikode Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Kozhikode RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedKozhikodeRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Kozhikode RGBA Masterplan tile generation completed!")
