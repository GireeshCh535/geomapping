#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Ajmer Master Development Plan RGBA GeoTIFF
...
class AjmerRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Ajmer Master Development Plan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/rajasthan/ajmer",
                 output_dir: str = "ajmer_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Ajmer RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=4, max_zoom=18):
        """Generate PNG tiles for Ajmer Master Development Plan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Ajmer Master Development Plan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Rajasthan - Ajmer Master Development Plan",
            "sources": {
                "ajmer-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/rajasthan/ajmer_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "ajmer-masterplan-layer",
                    "type": "raster",
                    "source": "ajmer-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Rajasthan - Ajmer Master Development Plan",
            "description": "Master development plan tiles for Ajmer, Rajasthan",
            "version": "1.0.0",
            "attribution": "Ajmer Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Rajasthan - Ajmer Master Development Plan</title>
...
def main():
    """Main function"""
    logger.info("Starting Ajmer RGBA Master Development Plan tile generation")
    
    # Initialize generator
    generator = AjmerRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=4, max_zoom=18)
    
    logger.info("Ajmer RGBA Master Development Plan tile generation completed!")
