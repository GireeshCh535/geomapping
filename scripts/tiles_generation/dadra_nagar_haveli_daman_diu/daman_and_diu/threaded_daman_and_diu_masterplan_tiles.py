#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Daman and Diu Masterplan RGBA GeoTIFF
...
class ThreadedDamanAndDiuRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Daman and Diu Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/dadra-nagar-haveli-daman-diu/dadra-nagar-haveli-daman-diu/daman_and_diu_masterplan",
                output_dir: str = "daman_and_diu_masterplan_tiles",
        ...
        logger.info(f"Threaded Daman and Diu RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Daman and Diu Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Daman and Diu Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Daman and Diu Masterplan (Threaded)",
            "sources": {
                "daman-and-diu-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/dadra-nagar-haveli-daman-diu/daman_and_diu_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "daman-and-diu-masterplan-layer",
                    "type": "raster",
                    "source": "daman-and-diu-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Daman and Diu Masterplan (Threaded)",
            "description": "Master plan tiles for Daman and Diu (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Daman and Diu Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Daman and Diu Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Daman and Diu RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedDamanAndDiuRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Daman and Diu RGBA Masterplan tile generation completed!")
