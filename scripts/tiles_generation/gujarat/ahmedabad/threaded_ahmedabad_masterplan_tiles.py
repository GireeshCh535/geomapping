#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Ahmedabad Masterplan RGBA GeoTIFF
...
class ThreadedAhmedabadRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Ahmedabad Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/gujarat/ahmedabad-gandhinagar/ahmedabad_masterplan",
                 output_dir: str = "ahmedabad_masterplan_tiles",
        ...
        logger.info(f"Threaded Ahmedabad RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Ahmedabad Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Ahmedabad Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Gujarat - Ahmedabad Masterplan (Threaded)",
            "sources": {
                "ahmedabad-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/gujarat/ahmedabad_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "ahmedabad-masterplan-layer",
                    "type": "raster",
                    "source": "ahmedabad-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Gujarat - Ahmedabad Masterplan (Threaded)",
            "description": "Master plan tiles for Gujarat - Ahmedabad (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Ahmedabad Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Gujarat - Ahmedabad Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Ahmedabad RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedAhmedabadRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=16, max_zoom=18)
    
    logger.info("Threaded Ahmedabad RGBA Masterplan tile generation completed!")
