#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from GIFT City Masterplan RGBA GeoTIFF
...
class ThreadedGiftCityRGBATileGenerator:
    """
    Generate high-quality PNG tiles from GIFT City Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/gujarat/gift-city/gift_city_masterplan",
                 output_dir: str = "gift_city_masterplan_tiles",
        ...
        logger.info(f"Threaded GIFT City RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for GIFT City Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for GIFT City Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Gujarat - GIFT City Masterplan (Threaded)",
            "sources": {
                "gift-city-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/gujarat/gift_city_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "gift-city-masterplan-layer",
                    "type": "raster",
                    "source": "gift-city-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Gujarat - GIFT City Masterplan (Threaded)",
            "description": "Master plan tiles for Gujarat - GIFT City (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "GIFT City Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Gujarat - GIFT City Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded GIFT City RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedGiftCityRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded GIFT City RGBA Masterplan tile generation completed!")
