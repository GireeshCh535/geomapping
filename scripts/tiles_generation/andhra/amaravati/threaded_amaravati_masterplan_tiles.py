#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Amaravati Masterplan RGBA GeoTIFF
...
class ThreadedAmaravatiMasterplanTileGenerator:
    """
    Generate high-quality PNG tiles from Amaravati Masterplan RGBA GeoTIFF with multi-threading
    ...
    def __init__(self, data_dir: str = "data/andhra_pradesh/amaravati/master_plan",
        ...
        logger.info(f"Threaded Amaravati Masterplan Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Amaravati Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Amaravati Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Andhra Pradesh - Amaravati Masterplan (Threaded)",
            "sources": {
                "amaravati-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/andhra-pradesh/amaravati_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "amaravati-masterplan-layer",
                    "type": "raster",
                    "source": "amaravati-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Andhra Pradesh - Amaravati Masterplan (Threaded)",
            "description": "Master plan tiles for Andhra Pradesh - Amaravati (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
...
<html>
<head>
    <title>Andhra Pradesh - Amaravati Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Amaravati Masterplan tile generation")

    # Initialize generator with configurable workers
    generator = ThreadedAmaravatiMasterplanTileGenerator(max_workers=8)

    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)

    logger.info("Threaded Amaravati Masterplan tile generation completed!")
