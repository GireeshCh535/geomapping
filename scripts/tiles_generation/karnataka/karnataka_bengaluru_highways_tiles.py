...
class KarnatakaBengaluruHighwaysTileGenerator:
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        ...
        # Count existing tiles before starting
        existing_tiles = 0
        for zoom in range(min_zoom, max_zoom + 1):
            ...
            if zoom_dir.exists():
                for x_dir in zoom_dir.iterdir():
                    if x_dir.is_dir():
                        existing_tiles += len(list(x_dir.glob("*.png")))
        
        if existing_tiles > 0:
            logger.info(f"Found {existing_tiles} existing tiles - these will be skipped")
        ...
        for zoom in range(min_zoom, max_zoom + 1):
            ...
            for x in range(min_tile.x, max_tile.x + 1):
                ...
                for y in range(max_tile.y, min_tile.y + 1):
                    ...
                    result = self.generate_single_tile(all_features, zoom, x, y, tile_path)
                    if result == "generated":
                        total_tiles += 1
                    elif result == "skipped":
                        skipped_tiles += 1
                    ...
                    if (total_tiles + skipped_tiles) % 100 == 0:
                        logger.info(f"Processed {total_tiles + skipped_tiles} tiles so far (generated: {total_tiles}, skipped: {skipped_tiles})...")

        logger.info(f"Tile generation completed: {total_tiles} new tiles generated, {skipped_tiles} tiles skipped (already existed)")
        ...
        try:
            ...
            for i in range(len(pixel_coords) - 1):
                ...
                if (0 <= start[0] <= 256 and 0 <= start[1] <= 256 and
                    ...
                    draw.line([start, end], fill=self.highway_color, width=3)
