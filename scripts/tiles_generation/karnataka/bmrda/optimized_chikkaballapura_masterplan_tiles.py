#!/usr/bin/env python3
"""
Optimized script to generate high-quality PNG tiles from Chikkaballapura Masterplan RGBA GeoTIFF
...
class OptimizedChikkaballapuraMasterplanTileGenerator:
    """
    Generate high-quality PNG tiles from Chikkaballapura Masterplan RGBA GeoTIFF with optimized performance
    """
    
    def __init__(self, data_dir: str = "data/karnataka/BMRDA/Chikkaballapura Masterplan",
                 output_dir: str = "chikkaballapura_masterplan_tiles",
        ...
        logger.info(f"Chikkaballapura Masterplan Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Chikkaballapura Masterplan with optimized performance"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Chikkaballapura Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Karnataka BMRDA - Chikkaballapura Masterplan (Optimized)",
            "sources": {
                "chikkaballapura-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/karnataka/bmrda/chikkaballapura_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "chikkaballapura-masterplan-layer",
                    "type": "raster",
                    "source": "chikkaballapura-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Karnataka BMRDA - Chikkaballapura Masterplan (Optimized)",
            "description": "Master plan tiles for Karnataka BMRDA - Chikkaballapura (Optimized for performance)",
            "version": "1.0.0",
            "attribution": "BMRDA",
            "template": "",
            "legend": "",
...
<html>
<head>
    <title>Karnataka BMRDA - Chikkaballapura Masterplan (Optimized)</title>
...
<body>
    ...
    <div class="info">
        <h3>Chikkaballapura Masterplan</h3>
...
def main():
    """Main function"""
    logger.info("Starting Optimized Chikkaballapura Masterplan tile generation")
    
    # Initialize generator with optimized settings
    generator = OptimizedChikkaballapuraMasterplanTileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=18, max_zoom=18)
    
    logger.info("Optimized Chikkaballapura Masterplan tile generation completed!")
