#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Mohali SAS Nagar Masterplan RGBA GeoTIFF
...
class ThreadedMohaliSasNagarRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Mohali SAS Nagar Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/punjab/mohali-sas-nagar/mohali_sas_nagar_masterplan",
                 output_dir: str = "mohali_sas_nagar_masterplan_tiles",
        ...
        logger.info(f"Threaded Mohali SAS Nagar RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Mohali SAS Nagar Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Mohali SAS Nagar Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Punjab - Mohali SAS Nagar Masterplan (Threaded)",
            "sources": {
                "mohali-sas-nagar-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/punjab/mohali_sas_nagar_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "mohali-sas-nagar-masterplan-layer",
                    "type": "raster",
                    "source": "mohali-sas-nagar-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Punjab - Mohali SAS Nagar Masterplan (Threaded)",
            "description": "Master plan tiles for Punjab - Mohali SAS Nagar (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Punjab Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Punjab - Mohali SAS Nagar Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Mohali SAS Nagar RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedMohaliSasNagarRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Mohali SAS Nagar RGBA Masterplan tile generation completed!")
