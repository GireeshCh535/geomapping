#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Derabassi Masterplan RGBA GeoTIFF
...
class ThreadedDerabassiRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Derabassi Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/punjab/mohali-sas-nagar/derabassi_masterplan",
                 output_dir: str = "derabassi_masterplan_tiles",
        ...
        logger.info(f"Threaded Derabassi RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Derabassi Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Derabassi Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Punjab - Derabassi Masterplan (Threaded)",
            "sources": {
                "derabassi-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/punjab/mohali-sas-nagar/derabassi_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "derabassi-masterplan-layer",
                    "type": "raster",
                    "source": "derabassi-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Punjab - Derabassi Masterplan (Threaded)",
            "description": "Master plan tiles for Punjab - Derabassi (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Derabassi Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Punjab - Derabassi Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Derabassi RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedDerabassiRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Derabassi RGBA Masterplan tile generation completed!")
