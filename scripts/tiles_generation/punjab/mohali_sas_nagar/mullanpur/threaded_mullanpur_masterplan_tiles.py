#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Mullanpur Masterplan RGBA GeoTIFF
...
class ThreadedMullanpurRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Mullanpur Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/punjab/mohali-sas-nagar/mullanpur_masterplan",
                 output_dir: str = "mullanpur_masterplan_tiles",
        ...
        logger.info(f"Threaded Mullanpur RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Mullanpur Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Mullanpur Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Punjab - Mullanpur Masterplan (Threaded)",
            "sources": {
                "mullanpur-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/punjab/mohali-sas-nagar/mullanpur_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "mullanpur-masterplan-layer",
                    "type": "raster",
                    "source": "mullanpur-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Punjab - Mullanpur Masterplan (Threaded)",
            "description": "Master plan tiles for Punjab - Mullanpur (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Mullanpur Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Punjab - Mullanpur Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Mullanpur RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedMullanpurRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Mullanpur RGBA Masterplan tile generation completed!")
