#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Banur Masterplan RGBA GeoTIFF
...
class ThreadedBanurRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Banur Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/punjab/mohali-sas-nagar/banur_masterplan",
                 output_dir: str = "banur_masterplan_tiles",
        ...
        logger.info(f"Threaded Banur RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Banur Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Banur Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Punjab - Banur Masterplan (Threaded)",
            "sources": {
                "banur-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/punjab/mohali-sas-nagar/banur_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "banur-masterplan-layer",
                    "type": "raster",
                    "source": "banur-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Punjab - Banur Masterplan (Threaded)",
            "description": "Master plan tiles for Punjab - Banur (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Banur Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Punjab - Banur Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Banur RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedBanurRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Banur RGBA Masterplan tile generation completed!")
