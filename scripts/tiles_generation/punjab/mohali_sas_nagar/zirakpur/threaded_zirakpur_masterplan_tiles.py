#!/usr/bin/env python3
"""
Multi-threaded script to generate high-quality PNG tiles from Zirakpur Masterplan RGBA GeoTIFF
...
class ThreadedZirakpurRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Zirakpur Masterplan RGBA GeoTIFF with multi-threading
    Maintains pixel-by-pixel rendering for maximum quality
    """
    
    def __init__(self, data_dir: str = "data/punjab/mohali-sas-nagar/zirakpur_masterplan",
                 output_dir: str = "zirakpur_masterplan_tiles",
        ...
        logger.info(f"Threaded Zirakpur RGBA Tile Generator initialized with {self.max_workers} workers")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Zirakpur Masterplan with multi-threading"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Zirakpur Masterplan in {duration:.2f} seconds")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Punjab - Zirakpur Masterplan (Threaded)",
            "sources": {
                "zirakpur-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/punjab/mohali-sas-nagar/zirakpur_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "zirakpur-masterplan-layer",
                    "type": "raster",
                    "source": "zirakpur-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Punjab - Zirakpur Masterplan (Threaded)",
            "description": "Master plan tiles for Punjab - Zirakpur (Multi-threaded with pixel-perfect quality)",
            "version": "1.0.0",
            "attribution": "Zirakpur Urban Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Punjab - Zirakpur Masterplan (Threaded)</title>
...
def main():
    """Main function"""
    logger.info("Starting Threaded Zirakpur RGBA Masterplan tile generation")
    
    # Initialize generator with configurable workers
    generator = ThreadedZirakpurRGBATileGenerator(max_workers=8)
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Threaded Zirakpur RGBA Masterplan tile generation completed!")
