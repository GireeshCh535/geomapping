#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Mandideep Masterplan RGBA GeoTIFF
...
class MandideepRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Mandideep Masterplan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/madhya-pradesh/mandideep/mandideep_masterplan",
                 output_dir: str = "mandideep_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Mandideep RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Mandideep Masterplan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Mandideep Masterplan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Madhya Pradesh - Mandideep Masterplan",
            "sources": {
                "mandideep-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/madhya-pradesh/mandideep/mandideep_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "mandideep-masterplan-layer",
                    "type": "raster",
                    "source": "mandideep-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Madhya Pradesh - Mandideep Masterplan",
            "description": "Master plan tiles for Madhya Pradesh - Mandideep",
            "version": "1.0.0",
            "attribution": "Madhya Pradesh Government",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Madhya Pradesh - Mandideep Masterplan</title>
...
def main():
    """Main function"""
    logger.info("Starting Mandideep RGBA Masterplan tile generation")
    
    # Initialize generator
    generator = MandideepRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=4, max_zoom=18)
    
    logger.info("Mandideep RGBA Masterplan tile generation completed!")
