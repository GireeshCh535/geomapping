#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Bhopal Land Use Plan RGBA GeoTIFF
...
class BhopalRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Bhopal Land Use Plan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/madhya-pradesh/bhopal",
                 output_dir: str = "bhopal_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Bhopal RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=4, max_zoom=18):
        """Generate PNG tiles for Bhopal Land Use Plan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Bhopal Land Use Plan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Madhya Pradesh - Bhopal Land Use Plan",
            "sources": {
                "bhopal-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/madhya-pradesh/bhopal_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "bhopal-masterplan-layer",
                    "type": "raster",
                    "source": "bhopal-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Madhya Pradesh - Bhopal Land Use Plan",
            "description": "Land use plan tiles for Bhopal, Madhya Pradesh",
            "version": "1.0.0",
            "attribution": "Bhopal Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Madhya Pradesh - Bhopal Land Use Plan</title>
...
def main():
    """Main function"""
    logger.info("Starting Bhopal RGBA Land Use Plan tile generation")
    
    # Initialize generator
    generator = BhopalRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=4, max_zoom=18)
    
    logger.info("Bhopal RGBA Land Use Plan tile generation completed!")
