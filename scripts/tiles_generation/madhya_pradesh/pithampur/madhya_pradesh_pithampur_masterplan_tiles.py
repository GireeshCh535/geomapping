#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Pithampur Land Use Plan RGBA GeoTIFF
...
class PithampurRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Pithampur Land Use Plan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/madhya-pradesh/pithampur",
                 output_dir: str = "pithampur_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Pithampur RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=4, max_zoom=18):
        """Generate PNG tiles for Pithampur Land Use Plan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Pithampur Land Use Plan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Madhya Pradesh - Pithampur Land Use Plan",
            "sources": {
                "pithampur-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/madhya-pradesh/pithampur_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "pithampur-masterplan-layer",
                    "type": "raster",
                    "source": "pithampur-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Madhya Pradesh - Pithampur Land Use Plan",
            "description": "Land use plan tiles for Pithampur, Madhya Pradesh",
            "version": "1.0.0",
            "attribution": "Pithampur Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Madhya Pradesh - Pithampur Land Use Plan</title>
...
def main():
    """Main function"""
    logger.info("Starting Pithampur RGBA Land Use Plan tile generation")
    
    # Initialize generator
    generator = PithampurRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=4, max_zoom=18)
    
    logger.info("Pithampur RGBA Land Use Plan tile generation completed!")
