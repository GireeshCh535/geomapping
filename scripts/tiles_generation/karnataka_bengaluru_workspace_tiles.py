#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Karnataka Bengaluru workspace GeoJSON
Creates tiles with specified color: #AA66B2
"""
...
class KarnatakaBengaluruWorkspaceTileGenerator:
    """
    Generate high-quality PNG tiles from Karnataka Bengaluru workspace GeoJSON
    """

    def __init__(self, data_dir: str = "data/karnataka/bengaluru/workspace",
                 output_dir: str = "karnataka_bengaluru_workspace_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)

        # Color specification
        self.workspace_color = (170, 102, 178)  # #AA66B2

        logger.info("Karnataka Bengaluru Workspace Tile Generator initialized")

    def load_geojson_workspace(self):
        """Load and parse the GeoJSON workspace"""
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Karnataka Bengaluru workspace"""
        # Load GeoJSON workspace
        features_data = self.load_geojson_workspace()
        if features_data is None:
            logger.error("Failed to load GeoJSON workspace")
            return
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Karnataka Bengaluru workspace")
    ...
    def generate_single_tile(self, all_features, zoom, x, y, tile_path):
        ...
        try:
            ...
            # Render the workspace to this tile
            self.render_workspace_to_tile(all_features, tile_bounds, draw)
    ...
    def render_workspace_to_tile(self, all_features, tile_bounds, draw):
        """Render workspace to a tile"""
        try:
            ...
            for feature in all_features:
                ...
                if geometry['type'] == 'Polygon':
                    self.draw_polygon(geometry['coordinates'], tile_bounds, tile_width, tile_height, draw)
                elif geometry['type'] == 'MultiPolygon':
                    for polygon in geometry['coordinates']:
                        self.draw_polygon(polygon, tile_bounds, tile_width, tile_height, draw)

        except Exception as e:
            logger.error(f"Error rendering workspace to tile: {e}")

    def draw_polygon(self, coordinates, tile_bounds, tile_width, tile_height, draw):
        """Draw a Polygon on the tile"""
        try:
            # Handle both single polygon and multi-polygon coordinates
            if len(coordinates) > 0 and isinstance(coordinates[0][0], (list, tuple)):
                # Multi-polygon or polygon with holes
                for ring in coordinates:
                    self.draw_ring(ring, tile_bounds, tile_width, tile_height, draw)
            else:
                # Single ring
                self.draw_ring(coordinates, tile_bounds, tile_width, tile_height, draw)

        except Exception as e:
            logger.error(f"Error drawing Polygon: {e}")

    def draw_ring(self, coordinates, tile_bounds, tile_width, tile_height, draw):
        """Draw a ring (polygon outline or hole) on the tile"""
        try:
            tile_coords = []
            ...
            if len(tile_coords) >= 3:
                # Fill the polygon
                draw.polygon(tile_coords, fill=self.workspace_color)
                # Draw the outline
                draw.line(tile_coords + [tile_coords[0]], fill=self.workspace_color, width=1)

        except Exception as e:
            logger.error(f"Error drawing ring: {e}")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Karnataka - Bengaluru Workspace",
            "sources": {
                "karnataka-bengaluru-workspace": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/karnataka/bengaluru/workspace/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "karnataka-bengaluru-workspace-layer",
                    "type": "raster",
                    "source": "karnataka-bengaluru-workspace",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Karnataka - Bengaluru Workspace",
            "description": "Industrial areas and workspaces in Bengaluru, Karnataka",
            "version": "1.0.0",
...
<html>
<head>
    <title>Karnataka - Bengaluru Workspace</title>
...
def main():
    """Main function"""
    logger.info("Starting Karnataka Bengaluru workspace tile generation")
