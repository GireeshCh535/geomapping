#!/usr/bin/env python3
"""
Dedicated script to generate high-quality PNG tiles from Srinagar Masterplan RGBA GeoTIFF
...
class SrinagarRGBATileGenerator:
    """
    Generate high-quality PNG tiles from Srinagar Masterplan RGBA GeoTIFF
    """
    
    def __init__(self, data_dir: str = "data/jammu-kashmir/srinagar/srinagar_masterplan",
                 output_dir: str = "srinagar_masterplan_tiles"):
        self.data_dir = Path(data_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        logger.info("Srinagar RGBA Tile Generator initialized")
    ...
    def generate_tiles(self, min_zoom=8, max_zoom=16):
        """Generate PNG tiles for Srinagar Masterplan"""
        ...
        logger.info(f"Generated {total_tiles} PNG tiles for Srinagar Masterplan")
    ...
    def create_supporting_files(self, bounds, min_zoom, max_zoom):
        ...
        style_json = {
            "version": 8,
            "name": "Jammu & Kashmir - Srinagar Masterplan",
            "sources": {
                "srinagar-masterplan": {
                    "type": "raster",
                    "tiles": [
                        "https://d17yosovmfjm4.cloudfront.net/jammu-kashmir/srinagar_masterplan/{z}/{x}/{y}.png"
                    ],
                    "tileSize": 256
                }
            },
            "layers": [
                {
                    "id": "srinagar-masterplan-layer",
                    "type": "raster",
                    "source": "srinagar-masterplan",
                    "paint": {
                        "raster-opacity": 0.8
                    }
                }
        ...
        tilejson = {
            "tilejson": "2.2.0",
            "name": "Jammu & Kashmir - Srinagar Masterplan",
            "description": "Master plan tiles for Srinagar, Jammu & Kashmir",
            "version": "1.0.0",
            "attribution": "Srinagar Development Authority",
            "template": "",
            "legend": "",
            "scheme": "xyz",
            "tiles": [
...
<html>
<head>
    <title>Jammu & Kashmir - Srinagar Masterplan</title>
...
def main():
    """Main function"""
    logger.info("Starting Srinagar RGBA Masterplan tile generation")
    
    # Initialize generator
    generator = SrinagarRGBATileGenerator()
    
    # Generate tiles with higher zoom levels for better quality
    generator.generate_tiles(min_zoom=5, max_zoom=18)
    
    logger.info("Srinagar RGBA Masterplan tile generation completed!")
