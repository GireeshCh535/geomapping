#!/usr/bin/env python3
"""
GeoTIFF Analysis Script for Tirupati Master Plan
...
def check_tirupati_geotiff(tiff_path):
    """Analyze Tirupati GeoTIFF file"""
...
def main():
    """Main function"""
    tirupati_tif_path = Path("data/andhra-pradesh/tirupati/tirupati_masterplan")
    
    # Find TIF files in the directory
    tif_files = list(tirupati_tif_path.glob("*.tif")) + list(tirupati_tif_path.glob("*.tiff"))
    
    if not tif_files:
        logger.error(f"No TIF files found in {tirupati_tif_path}")
        return
    ...
    for tif_file in tif_files:
        ...
        logger.info(f"{'='*60}")
        check_tirupati_geotiff(tif_file)
