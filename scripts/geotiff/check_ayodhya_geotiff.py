#!/usr/bin/env python3
"""
GeoTIFF Analysis Script for Ayodhya Master Plan
...
def check_ayodhya_geotiff(tiff_path):
    """Analyze Ayodhya GeoTIFF file"""
...
def main():
    """Main function"""
    ayodhya_tif_path = Path("data/uttar-pradesh/ayodhya")
    
    # Find TIF files in the directory
    tif_files = list(ayodhya_tif_path.glob("*.tif")) + list(ayodhya_tif_path.glob("*.tiff"))
    
    if not tif_files:
        logger.error(f"No TIF files found in {ayodhya_tif_path}")
        return
    ...
    for tif_file in tif_files:
        ...
        logger.info(f"{'='*60}")
        check_ayodhya_geotiff(tif_file)
