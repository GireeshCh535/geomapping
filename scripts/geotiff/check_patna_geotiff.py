#!/usr/bin/env python3
"""
GeoTIFF Analysis Script for Patna Master Plan
...
def check_patna_geotiff(tiff_path):
    """Analyze Patna GeoTIFF file"""
...
def main():
    """Main function"""
    patna_tif_path = Path("data/bihar/patna")
    
    # Find TIF files in the directory
    tif_files = list(patna_tif_path.glob("*.tif")) + list(patna_tif_path.glob("*.tiff"))
    
    if not tif_files:
        logger.error(f"No TIF files found in {patna_tif_path}")
        return
