#!/usr/bin/env python3
"""
GeoTIFF Analysis Script for Lucknow Master Plan
...
def check_lucknow_geotiff(tiff_path):
    """Analyze Lucknow GeoTIFF file"""
...
def main():
    """Main function"""
    lucknow_tif_path = Path("data/uttar-pradesh/lucknow")
    
    # Find TIF files in the directory
    tif_files = list(lucknow_tif_path.glob("*.tif")) + list(lucknow_tif_path.glob("*.tiff"))
    
    if not tif_files:
        logger.error(f"No TIF files found in {lucknow_tif_path}")
        return
    ...
    for tif_file in tif_files:
        ...
        logger.info(f"{'='*60}")
        check_lucknow_geotiff(tif_file)
