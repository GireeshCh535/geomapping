#!/usr/bin/env python3
"""
Comprehensive Analysis of Warangal Master Plan GeoJSON Files for Tile Generation
This script analyzes all GeoJSON files to determine optimal tile generation parameters.
"""
...
def analyze_geojson_file(filepath):
    ...
    try:
        ...
        for key in sorted(property_keys)[:20]:  # Show first 20
            print(f"  - {key}")
        if len(property_keys) > 20:
            print(f"  ... and {len(property_keys) - 20} more")
        ...
        if overall_bbox:
            ...
            for zoom in [10, 11, 12, 13, 14, 15, 16, 17, 18]:
        ...
        # Sample feature properties for deeper analysis
        if features:
...
def generate_summary_report(all_results):
    ...
    print(f"\n\n{'#'*80}")
    print(f"{'#'*80}")
    print(f"{'COMPREHENSIVE SUMMARY REPORT':^80}")
    print(f"{'#'*80}")
    print(f"{'#'*80}\n")
    ...
    for result in sorted_by_size[:10]:
        print(f"  {result['filename']:40s} - {result['file_size_mb']:8.2f} MB - {result['feature_count']:8,} features")
    
    # Files by feature count
    print(f"\nFiles by Feature Count (Most Features First):")
    sorted_by_features = sorted(valid_results, key=lambda x: x['feature_count'], reverse=True)
    for result in sorted_by_features[:10]:
        print(f"  {result['filename']:40s} - {result['feature_count']:8,} features")
    ...
    for result in valid_results:
        for prop in result['property_keys']:
            property_frequency[prop] += 1
    
    print(f"\nMost Common Properties (appearing in multiple files):")
    for prop, freq in sorted(property_frequency.items(), key=lambda x: x[1], reverse=True)[:20]:
        print(f"  {prop:30s} - appears in {freq}/{total_files} files")
    ...
    if overall_bbox:
        ...
        print(f"   - Overview: Zoom 10-12 (good for city-wide view)")
        print(f"   - Standard: Zoom 13-15 (good for neighborhood detail)")
        print(f"   - Detailed: Zoom 16-18 (good for plot/parcel level)")
        ...
        print(f"\n5. LAYER CATEGORIES:")
        layer_categories = sorted(set(r['filename'].replace('.geojson', '') for r in valid_results))
        print(f"   Total Layers: {len(layer_categories)}")
        for i, category in enumerate(layer_categories, 1):
            print(f"   {i:2d}. {category}")
        
        print(f"\n6. DATA CHARACTERISTICS:")
        print(f"   - Total Features: {total_features:,}")
        print(f"   - Average Complexity: {sum(r['total_coordinates'] for r in valid_results) / total_features:.1f} coords/feature")
        print(f"   - Coordinate Reference System: EPSG:4326 (WGS84)")
        
        print(f"\n7. RENDERING CONSIDERATIONS:")
        print(f"   - Files with >10k features may need feature simplification")
        print(f"   - Consider separate tile generation for each layer category")
        print(f"   - Use clustering for dense point data")
        print(f"   - Implement progressive loading for high zoom levels")
        
        print(f"\n8. SUGGESTED TILE GENERATION PARAMETERS:")
        print(f"   MIN_ZOOM = 10")
        print(f"   MAX_ZOOM = 18")
        print(f"   TILE_SIZE = 256  # pixels")
        print(f"   BUFFER_SIZE = 64  # pixels (to handle features at tile edges)")
        print(f"   SIMPLIFICATION = True  # Simplify geometries at lower zooms")
        print(f"   FORMAT = 'vector'  # MVT (Mapbox Vector Tiles) recommended")

def main():
    """Main analysis function"""
    base_path = Path("/Users/rohitboni/Downloads/All_files/project/1acre/geomapping_full/geomapping")
    data_path = base_path / "data" / "Telangana" / "warangal" / "master_plan"
