#!/usr/bin/env python3
"""
Comprehensive Analysis of Amaravati Master Plan GeoJSON Files for Tile Generation
Analyzes all 41 layers to determine optimal tile generation parameters.
"""
...
def generate_summary_report(all_results):
    ...
    print(f"\n\n{'#'*80}")
    print(f"{'#'*80}")
    print(f"{'AMARAVATI MASTER PLAN - COMPREHENSIVE SUMMARY':^80}")
    print(f"{'#'*80}")
    print(f"{'#'*80}\n")
    ...
    for i, result in enumerate(sorted_by_size[:15], 1):
        print(f"  {i:2d}. {result['filename']:45s} - {result['file_size_mb']:8.2f} MB - {result['feature_count']:8,} features")
    ...
    for i, result in enumerate(sorted_by_features[:15], 1):
        print(f"  {i:2d}. {result['filename']:45s} - {result['feature_count']:8,} features")
    ...
    for result in valid_results:
        ...
        if any(x in name for x in ['R1', 'R3', 'R4', 'SR', 'Residential']):
            categories['Residential'].append(name)
        elif any(x in name for x in ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'Commercial']):
            categories['Commercial'].append(name)
        elif any(x in name for x in ['I1', 'I2', 'I3', 'Industrial']):
            categories['Industrial'].append(name)
        elif any(x in name for x in ['S2', 'S3', 'SS', 'Education', 'Health', 'Cultural', 'Government']):
            categories['Public/Semi-Public'].append(name)
        elif any(x in name for x in ['P1', 'P2', 'P3', 'SP', 'Protected', 'Passive', 'Active']):
            categories['Protected/Open'].append(name)
        elif any(x in name for x in ['U1', 'U2', 'SU', 'Reserve', 'Road', 'PGN']):
            categories['Utilities/Infrastructure'].append(name)
        elif any(x in name for x in ['Vacant']):
            categories['Vacant'].append(name)
        else:
            categories['Special/Mixed'].append(name)
    
    for category, zones in categories.items():
        if zones:
            print(f"{category} ({len(zones)} zones):")
    ...
    if overall_bbox:
        ...
        print(f"   Consider grouping by category for better performance")
        
        print(f"\n6. DATA CHARACTERISTICS:")
        print(f"   - Total Features: {total_features:,}")
        print(f"   - Average Complexity: {sum(r['total_coordinates'] for r in valid_results) / total_features:.1f} coords/feature")
        print(f"   - More complex than typical datasets - careful optimization needed")
        ...
        if complexity_high:
            ...
            for r in sorted(complexity_high, key=lambda x: x['total_coordinates'] / x['feature_count'], reverse=True)[:5]:
...
def main():
    ...
    data_path = base_path / "data" / "andhra_pradesh" / "amaravati" / "master_plan"
