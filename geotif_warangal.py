...
OUTPUT_FILE = "warangal_masterplan_zoom16.tif"

# Resolution in meters per pixel (zoom level 16 = 2.4m/pixel)
TARGET_RESOLUTION_METERS = 2.4  # Adjust this value for different detail levels

# Common values and their characteristics:
#   2.4m  = Zoom 16 (very high detail) → ~28,000 × 22,000 px, ~500 MB file
#   4.8m  = Zoom 15 (high detail)      → ~14,000 × 11,000 px, ~120 MB file
#   9.6m  = Zoom 14 (medium detail)    →  ~7,000 ×  5,700 px,  ~30 MB file
#   19.1m = Zoom 13 (standard detail)  →  ~3,500 ×  2,800 px,   ~8 MB file
#   34.2m = Auto (fast preview)        →  ~2,000 ×  1,600 px,   ~2 MB file
...
def calculate_dimensions(bounds, target_resolution_meters):
    ...
    # Estimate file size (RGB = 3 bytes per pixel, with compression ~40%)
    uncompressed_mb = (width_px * height_px * 3) / (1024 * 1024)
    estimated_mb = uncompressed_mb * 0.4
    ...
    print(f"  Estimated file size: {estimated_mb:.1f} MB (compressed)")
...
def rasterize_rgb(layers, bounds, width, height):
    """Rasterize features as RGBA image with transparent background"""
    ...
    if total_pixels > 50_000_000:
        print(f"  ⚠️  Large image ({total_pixels:,} pixels) - this will take time...")
    ...
    a_band = np.zeros((height, width), dtype=np.uint8)  # Alpha channel (0 = transparent)
    ...
    for layer_name, layer_data in sorted_layers:
        ...
        print(f"  Rendering {layer_name}: {len(features)} features...")
        
        # For each color channel, rasterize with that channel's value
        for feature in features:
            r, g, b = feature['color']
            
            # Rasterize this feature for each band
            ...
            # Apply color to RGB channels where mask is 1
            r_band[mask == 1] = r
            g_band[mask == 1] = g
            b_band[mask == 1] = b
            a_band[mask == 1] = 255  # Set alpha to opaque (255)
...
def main():
    ...
    try:
        ...
        print("-" * 70)
        print("\nThe file has a transparent background and zoom 16 clarity!")
        print("Perfect for overlay on satellite imagery or web maps!")
