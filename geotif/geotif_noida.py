#!/usr/bin/env python3
"""
Noida Master Plan GeoJSON → High-Resolution RGB GeoTIFF Converter
"""
...
# CONFIGURATION
# ============================================================================
INPUT_DIR = "data/delhi_ncr/noida/master_plan"
OUTPUT_FILE = "noida_masterplan_zoom16.tif"
...
COLOR_MAP = {
    "5 % ABADI": hex_to_rgb("D7D79E"),
    "5% ABADI": hex_to_rgb("D7D79E"),
    "FIVE PERCENT ABADI": hex_to_rgb("D7D79E"),
    "FIVE_PERCENT_ABADI": hex_to_rgb("D7D79E"),
    "COMMERCIAL": hex_to_rgb("FF0000"),
    "FARMHOUSE": hex_to_rgb("FDB3FD"),
    "GREEN": hex_to_rgb("4CE600"),
    "GROUP HOUSING": hex_to_rgb("FFAA00"),
    "GROUP_HOUSING": hex_to_rgb("FFAA00"),
    "HOUSING": hex_to_rgb("FFEBAF"),
    "INDUSTRIAL": hex_to_rgb("6699CD"),
    "INDUSTRY": hex_to_rgb("6699CD"),
    "INSTITUTIONAL": hex_to_rgb("004DA8"),
    "ROAD": hex_to_rgb("CCCCCC"),
    "ROADS": hex_to_rgb("CCCCCC"),
    "RECREATIONAL GREEN": hex_to_rgb("55FF00"),
    "RECREATIONAL_GREEN": hex_to_rgb("55FF00"),
    "RESIDENTIAL": hex_to_rgb("FFFF00"),
    "SPORTS": hex_to_rgb("FF0000"),
    "TRANSPORT": hex_to_rgb("A87000"),
    "UTILITY": hex_to_rgb("9C9C9C"),
    "VILLAGE ABADI": hex_to_rgb("D7D79E"),
    "VILLAGE_ABADI": hex_to_rgb("D7D79E"),
    "WATER BODY": hex_to_rgb("0070FF"),
    "WATER_BODY": hex_to_rgb("0070FF"),
    "NA": hex_to_rgb("FFFFFF"),
    "NOT KNOWN": hex_to_rgb("FFFFFF"),
    "NOT_KNOWN": hex_to_rgb("FFFFFF"),
}
...
def load_features_by_layer(geojson_files):
    ...
    for path in geojson_files:
        rel_path = os.path.relpath(path, INPUT_DIR)
        layer_name = os.path.splitext(rel_path)[0].replace("/", "_").replace("\\", "_")
        ...
        for feat in data.get("features", []):
            try:
                ...
                if color is None:
                    color = (128, 128, 128)
                    unmapped.add(category_norm or layer_norm or layer_name)
...
def write_rgb_geotiff(rgba_data, transform, output_file):
    ...
    with rasterio.open(
        ...
        for idx in range(4):
            ...
            print(" done.")
        dst.update_tags(
            description="Noida Master Plan Land Use Map (RGBA)",
            created_by="geotif_noida.py",
            alpha_band="4",
            compression="lzw",
        )
...
def main():
    print("=" * 70)
    print("Noida GeoJSON → High-Resolution RGB GeoTIFF")
    print("=" * 70)
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("✓ SUCCESS: Noida master plan GeoTIFF created!")
        print("=" * 70)
        return 0
